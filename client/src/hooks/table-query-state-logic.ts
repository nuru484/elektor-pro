...
export type TableFilterFieldSpec =
   ...
   * (e.g. campaigns' isFeatured: featured / not-featured / any).
   */
 ...
 * is (de)serialized.
 */
export type TableFiltersSpec<TFilters extends Record<string, TableFilterValue>> = {
 ...
 * so it mirrors exactly what would appear in the URL (e.g. a `false` boolean
 * that doesn't serialize counts as unset). Drives the session-memory cleanup:
 * a table at its defaults has nothing worth remembering.
 */
