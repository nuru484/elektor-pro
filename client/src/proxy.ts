...
// next.config.ts rather than here. The CSP is deliberately not nonce-based:
// Next only stamps the nonce onto scripts of DYNAMICALLY rendered pages, and
// most pages here prerender at build time, so the served HTML carries 28
// script tags and none of them the nonce - under `script-src 'nonce-...'
// 'strict-dynamic'` the browser would block every one and serve a blank app.
// Making the whole app dynamic to satisfy the nonce would cost static
// rendering and CDN caching on the public pages.
